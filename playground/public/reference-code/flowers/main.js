...
var Prng = new (function () {
  ...
  this.hash = function (x) {
    let y = window.btoa(JSON.stringify(x));
    let z = 0;
...
function midPt() {
  let plist = arguments.length == 1 ? arguments[0] : Array.apply(null, arguments);
  return plist.reduce(
    (acc, v) => {
      return [
...
v3 = new (function () {
  ...
  this.lerp = function (u, v, p) {
    return [u.x * (1 - p) + v.x * p, u.y * (1 - p) + v.y * p, u.z * (1 - p) + v.z * p];
  };
})();
...
function paper(args) {
  ...
  for (let i = 0; i < reso / 2 + 1; i++) {
    for (let j = 0; j < reso / 2 + 1; j++) {
      ...
      var b = c * col[2];
      if (
        Noise.noise(i * 0.04, j * 0.04, 2) * Math.random() * spr > 0.7 ||
...
function leaf(args) {
  ...
  let col = args.col != undefined ? args.col : { min: [90, 0.2, 0.3, 1], max: [90, 0.1, 0.9, 1] };
  let cof = args.cof != undefined ? args.cof : (x) => x;
  let ben = args.ben != undefined ? args.ben : (x) => [normRand(-10, 10), 0, normRand(-5, 5)];
...
function stem(args) {
  ...
  let col = args.col != undefined ? args.col : { min: [250, 0.2, 0.4, 1], max: [250, 0.3, 0.6, 1] };
  let ben = args.ben != undefined ? args.ben : (x) => [normRand(-10, 10), 0, normRand(-5, 5)];
...
function branch(args) {
  ...
  let col = args.col != undefined ? args.col : { min: [50, 0.2, 0.8, 1], max: [50, 0.2, 0.8, 1] };
    ...
    if (typeof PAR[k] == "object") {
      ...
      for (var i in PAR[k]) {
        ...
        if (k.includes("olor")) {
          viz +=
            `<td ${tabstyle}>` +
            `<div style='background-color:${hsv(
              ...PAR[k][i],
...
function genParams() {
  console.log("🌲 Original genParams: First 5 random:", [
    Math.random(),
    Math.random(),
    Math.random(),
    Math.random(),
  ...
  PAR.flowerOpenCurve = randChoice([
    (x, op) => (x < 0.1 ? 2 + op * curveCoeff2[1] : Noise.noise(x * 10, curveCoeff2[0])),
    (x, op) => (x < curveCoeff4[0] ? 0 : 10 - x * mapval(op, 0, 1, 16, 20) * curveCoeff4[1]),
  ]);
...
function woody(args) {
  ...
  // Save params globally for comparison
  CURRENT_PARAMS = PAR;
  CURRENT_PLANT_TYPE = "woody";
  ...
  for (var i = 0; i < PAR.stemCount; i++) {
    ...
    if (PAR.sheathLength != 0) {
      stem({
        ...
        wid: (x) => PAR.sheathWidth * (sin(x * PI) ** 2 - x * 0.5 + 0.5),
        ben: (x) => [0, 0, 0],
      });
    }
...
// Global variable to store current params
let CURRENT_PARAMS = null;
let CURRENT_PLANT_TYPE = null;
...
function generate() {
  ...
  console.log("🔍 Original: First 3 random IMMEDIATELY after page load seed:", [
    Math.random(),
    Math.random(),
    Math.random(),
  ]);
  ...
  console.log("🌳 Original: Starting paper generation");
  console.log("🌳 Original: Random values BEFORE paper:", [
    Math.random(),
    Math.random(),
    Math.random(),
  ]);

  // Re-seed to get back to same state
  Math.seed(SEED);
  ...
  console.log("🌳 Original: Paper generation completed, random calls:", randomCount);
  for (let i = 0; i < CTX.canvas.width; i += 512) {
    for (let j = 0; j < CTX.canvas.height; j += 512) {
  ...
  console.log("🌳 Original Plant Type Random:", plantTypeRandom);
  if (plantTypeRandom <= 0.5) {
    console.log("🌳 Original Selected Type: woody");
    woody({ ctx: CTX, xof: 300, yof: 550 });
  } else {
    console.log("🌳 Original Selected Type: herbal");
...
// show current params in copyable format
function showParams() {
  if (!CURRENT_PARAMS) {
    alert("No params available. Generate a flower first!");
    return;
  }
  ...
  let output = `// Plant Type: ${CURRENT_PLANT_TYPE}\n`;
  output += `// Seed: ${SEED}\n\n`;
  output += `const params = {\n`;

  // Helper to format function
  ...
  for (let key in CURRENT_PARAMS) {
    output += formatValue(key, CURRENT_PARAMS[key]);
  }

  output += `}\n\n// Copy this to use in your implementation`;

  display.innerHTML = `<pre style="margin: 0; white-space: pre-wrap; word-wrap: break-word;">${output}</pre>
    <button onclick="copyParams()" style="margin-top: 10px; padding: 8px 16px; cursor: pointer; background: #42b983; color: white; border: none; border-radius: 4px;">Copy to Clipboard</button>`;
  display.style.display = "block";
}

// copy params to clipboard
function copyParams() {
  if (!CURRENT_PARAMS) return;

  let output = `// Plant Type: ${CURRENT_PLANT_TYPE}\n`;
  output += `// Seed: ${SEED}\n\n`;
  output += `const params = ${JSON.stringify(CURRENT_PARAMS, null, 2)}`;
  ...
  navigator.clipboard
    ...
    .catch((err) => {
      ...
      alert("Failed to copy. Please copy manually from the display.");
    });
}
...
function load() {
  ...
  function _load() {
    ...
    document.getElementById("share-twitter").href = `https://twitter.com/share?url=${
