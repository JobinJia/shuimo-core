...
export class WinterPlum {
  ...
  private static generateBranch(
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      const bend = Math.sin(t * Math.PI) * bendAmount * bendDir +
        noise.noise(t * 3, n0) * 0.15;

      const px = x + Math.cos(angle) * length * t +
        Math.cos(angle + Math.PI / 2) * length * bend;
      const py = y - Math.sin(angle) * length * t -
        Math.sin(angle + Math.PI / 2) * length * bend;
    ...
    for (let i = 0; i < subBranchCount; i++) {
      ...
      const subAngle = angle + (prng.next() < 0.5 ? 1 : -1) * (Math.PI / 4 + prng.next() * Math.PI / 4);
  ...
  private static generateBud(
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      points.push([
        x + Math.cos(budAngle) * budSize * t,
        y + Math.sin(budAngle) * budSize * t
      ]);
    }
