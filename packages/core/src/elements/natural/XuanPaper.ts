...
export class XuanPaper {
  ...
  private static generateGrainTexture(
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (r !== undefined)
          data[index] = Math.min(255, Math.max(0, r + variation))
        if (g !== undefined)
          data[index + 1] = Math.min(255, Math.max(0, g + variation))
        if (b !== undefined)
          data[index + 2] = Math.min(255, Math.max(0, b + variation))
      }
    }
  }
  ...
  static generate(options: XuanPaperOptions = {}): HTMLCanvasElement {
    ...
    if (goldFlecks) {
      this.generateGoldFlecks(ctx, width, height, goldDensity, goldSize, goldColor, goldClustering, seed, rng)
    }
  ...
  static generateSVG(options: XuanPaperOptions = {}): SVGSVGElement {
    ...
    // Add fiber texture
    if (fiberDensity > 0) {
      const fiberGroup = this.generateFiberLayerSVG(width, height, fiberDensity, fiberScale, seed, SVG_NS)
  ...
  private static generateFiberLayerSVG(
    ...
    for (let i = 0; i < fiberCount; i++) {
      ...
      path.setAttribute('stroke', `rgba(${brightness}, ${brightness - 10}, ${brightness - 20}, ${alpha})`)
    ...
    for (let i = 0; i < fragmentCount; i++) {
      ...
      path.setAttribute('d', d)
      path.setAttribute('fill', 'none')
      path.setAttribute('stroke', `rgba(${brightness}, ${brightness - 15}, ${brightness - 25}, ${alpha})`)
      path.setAttribute('stroke-width', strokeWidth.toString())
      path.setAttribute('stroke-linecap', 'round')
      group.appendChild(path)
    }
  ...
  private static generateGrainParticlesSVG(
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      circle.setAttribute('fill', `rgba(${brightness}, ${brightness - 5}, ${brightness - 10}, ${alpha})`)
    ...
    for (let i = 0; i < fragmentCount; i++) {
      ...
      ellipse.setAttribute('fill', `rgba(${brightness}, ${brightness - 8}, ${brightness - 15}, ${alpha})`)
