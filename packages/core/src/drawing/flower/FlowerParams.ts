...
export function genParams(): FlowerParams {
  ...
  const flowerColor = {
    min: [flowerHue0, flowerSaturation0, flowerValue0, normRand(0.8, 1)] as [
      number,
      number,
      number,
      number,
    ],
    max: [flowerHue1, flowerSaturation1, flowerValue1, normRand(0.5, 1)] as [
      number,
      number,
      number,
      number,
  ...
  // Leaf colors
  const leafColor = {
    min: [normRand(10, 200), normRand(0.05, 0.4), normRand(0.3, 0.7), normRand(0.8, 1)] as [
      number,
      number,
      number,
      number,
    ],
    max: [normRand(10, 200), normRand(0.05, 0.4), normRand(0.3, 0.7), normRand(0.8, 1)] as [
      number,
      number,
      number,
      number,
  ...
  const flowerOpenCurve = randChoice([
    (x: number, op: number) => (x < 0.1 ? 2 + op * curveCoeff2[1] : noise(x * 10, curveCoeff2[0])),
    (x: number, op: number) =>
      x < curveCoeff4[0] ? 0 : 10 - x * mapval(op, 0, 1, 16, 20) * curveCoeff4[1],
  ]);

  // Flower color curve
  const flowerColorCurve = randChoice([(x: number) => sigmoid(x + curveCoeff0[0], curveCoeff0[1])]);
  ...
  const innerColor = {
    min: [innerHue, normRand(0.1, 0.7), normRand(0.5, 0.9), normRand(0.8, 1)] as [
      number,
      number,
      number,
      number,
    ],
    max: [innerHue, normRand(0.1, 0.7), normRand(0.5, 0.9), normRand(0.5, 1)] as [
      number,
      number,
      number,
      number,
