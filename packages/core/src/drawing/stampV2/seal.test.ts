...
describe("generateSeal size-adaptive visual scaling (2.0.4-beta.1)", () => {
  // Same explicit carving + border config across three sizes; verify the
  // pipeline's actual emitted pixel constants scale proportionally so a
  // user-visible intensity reads the same "depth" at every size.
  //
  // We assert against the raw SVG output rather than introspecting internals
  // so the contract surfaces if someone re-wires the filter chain.
  const baseOpts = {
    text: "水墨",
    font: undefined as unknown as ArrayBuffer,
    seed: 7,
    mode: "yang" as const,
    border: { thickness: 6, roughness: 0.5 },
    carving: { intensity: 0.9 },
    ink: { bleed: 0.5 },
  };

  function extractTextFilterEdgeDisp(svg: string): number {
    // Match the textFilter's edgeDisp scale (first feDisplacementMap
    // following the textEdgeBand result).
    const match = svg.match(/textEdgeNoiseStepped"[^]*?scale="([0-9.]+)"/);
    if (!match) throw new Error("textFilter edge displacement not found in SVG");
    return parseFloat(match[1]);
  }

  function extractTextFilterEdgeFreq(svg: string): number {
    const match = svg.match(/<feTurbulence[^>]*baseFrequency="([0-9.]+)"[^>]*result="textEdgeNoise"/);
    if (!match) throw new Error("textFilter edge baseFrequency not found");
    return parseFloat(match[1]);
  }

  it("text-filter edge displacement halves when size halves", () => {
    const big = generateSeal({ ...baseOpts, font: fontBuf, size: 480 });
    const small = generateSeal({ ...baseOpts, font: fontBuf, size: 240 });
    const dispBig = extractTextFilterEdgeDisp(big.svg!);
    const dispSmall = extractTextFilterEdgeDisp(small.svg!);
    // 240 / 480 = 0.5, allow 5% slop for fmt() rounding to 3 decimals.
    expect(dispSmall / dispBig).toBeGreaterThan(0.48);
    expect(dispSmall / dispBig).toBeLessThan(0.52);
  });

  it("text-filter edge frequency doubles when size halves", () => {
    const big = generateSeal({ ...baseOpts, font: fontBuf, size: 480 });
    const small = generateSeal({ ...baseOpts, font: fontBuf, size: 240 });
    const freqBig = extractTextFilterEdgeFreq(big.svg!);
    const freqSmall = extractTextFilterEdgeFreq(small.svg!);
    expect(freqSmall / freqBig).toBeGreaterThan(1.95);
    expect(freqSmall / freqBig).toBeLessThan(2.05);
  });

  it("at the 480 reference size, frequency matches the V1 baseline", () => {
    const r = generateSeal({ ...baseOpts, font: fontBuf, size: 480 });
    const freq = extractTextFilterEdgeFreq(r.svg!);
    // V1 baseline = 0.28.
    expect(freq).toBeCloseTo(0.28, 2);
  });

  it("size 200 vs 600 spans a 3× scale on length, 1/3 on frequency", () => {
    const big = generateSeal({ ...baseOpts, font: fontBuf, size: 600 });
    const small = generateSeal({ ...baseOpts, font: fontBuf, size: 200 });
    const dispBig = extractTextFilterEdgeDisp(big.svg!);
    const dispSmall = extractTextFilterEdgeDisp(small.svg!);
    const freqBig = extractTextFilterEdgeFreq(big.svg!);
    const freqSmall = extractTextFilterEdgeFreq(small.svg!);
    // big / small = 600 / 200 = 3
    expect(dispBig / dispSmall).toBeGreaterThan(2.9);
    expect(dispBig / dispSmall).toBeLessThan(3.1);
    expect(freqSmall / freqBig).toBeGreaterThan(2.9);
    expect(freqSmall / freqBig).toBeLessThan(3.1);
  });
});
