...
export class StrokeTrajectoryEstimator {
  ...
  estimateTrajectory(shape: StrokeShape): DrawingTrajectory {
    ...
    const trajectoryPoints = this.adjustTrajectory(medialAxis, startType, endType, side1, side2);
   ...
   * 计算三次Bézier曲线上的点
   */
  private cubicBezier(p0: Vector2, p1: Vector2, p2: Vector2, p3: Vector2, t: number): Vector2 {
    const t2 = t * t;
    const t3 = t2 * t;
    const mt = 1 - t;
    const mt2 = mt * mt;
   ...
   * 找到最优终点（平滑端点情况）
   */
  private findOptimalEndPoint(medialAxis: Vector2[], side1: Vector2[], side2: Vector2[]): Vector2 {
    let maxLength = 0;
